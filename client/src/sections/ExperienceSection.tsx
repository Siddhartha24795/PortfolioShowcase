    ...
    <section id="experience" className="py-16 bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          {/* Timeline Line */}
          <div className="absolute left-0 md:left-1/2 ml-4 md:-ml-0.5 w-1 h-full bg-primary-200 dark:bg-primary-900"></div>
                  ...
                  {index % 2 === 0 ? (
                    ...
                    <>
                      <div className="md:w-1/2 mb-8 md:mb-0 md:pr-8 md:text-right">
                        <div className="hidden md:block absolute right-0 top-0 w-10 h-10 rounded-full bg-white dark:bg-gray-800 border-4 border-primary z-10 md:translate-x-1/2"></div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{experience.company}</h3>
                        <p className="text-primary-600 dark:text-primary-400 font-medium">{experience.title}</p>
                        <p className="text-gray-600 dark:text-gray-400 text-sm">{experience.date}</p>
                      </div>
                      <div className="md:w-1/2 pl-8 md:pl-12 relative">
                        <div className="md:hidden absolute left-0 top-0 w-8 h-8 rounded-full bg-white dark:bg-gray-800 border-4 border-primary z-10 -translate-x-1/2"></div>
                        <div className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md">
                          <h3 className="text-xl font-bold text-gray-900 dark:text-white md:hidden">{experience.company}</h3>
                          <p className="text-primary-600 dark:text-primary-400 font-medium md:hidden mb-1">{experience.title}</p>
                          <p className="text-gray-600 dark:text-gray-400 text-sm md:hidden mb-3">{experience.date}</p>
                            ...
                            <div className="mt-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {experience.technologies.map((tech, i) => (
                                  <span key={i} className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">
                                    {tech}
                                  </span>
                                ))}
                              </div>
                    ...
                    <>
                      ...
                      <div className="md:w-1/2 mb-8 md:mb-0 md:pl-8 order-1 md:order-2">
                        <div className="hidden md:block absolute left-0 top-0 w-10 h-10 rounded-full bg-white dark:bg-gray-800 border-4 border-primary z-10 md:translate-x-1/2"></div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{experience.company}</h3>
                        <p className="text-primary-600 dark:text-primary-400 font-medium">{experience.title}</p>
                        <p className="text-gray-600 dark:text-gray-400 text-sm">{experience.date}</p>
                      </div>
